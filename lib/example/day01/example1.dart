// example1.dart 파일 생성
// 1. void main(){} 이용한 최최 실행 코드 작성한다.
// 2. 위젯을 작성한다. 위젯이란? 화면을 구성하는 가장 작은 단위( == 컴포넌트 )

// [1] 위젯 만들기
//  (1) StatelessWidget 상속받는다.(상태없는 위젯을 상속받는다는 의미)
//  (2) build 함수 재구현한다.
...
class MyApp2 extends StatelessWidget{
  @override
  Widget build(BuildContext context) {
    // 자바 : new 클래스명(매개변수) vs DART : 클래스명/위젯명( 속성명 : 값 )
...
void main(){  // public static void main(){}와 비슷한 main스레드 갖는다.
  ...
  // runApp(MyApp1()); // runApp( 최초로실행할위젯 ); // 최초로 실행되는 위젯 실행, 리액트에서 App.jsx역할
  runApp( MyApp2() );
}
