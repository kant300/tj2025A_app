...
class GoodsWidgetState extends State<GoodsWidget>{
  ...
  void goodsSave () async{
    ...
    try{
      ...
      // ( web ) 빌드시 : localHost:8080 가능 , (mobile) :  192.168.40.133 가능
      // 스프링 컨트롤러 에서 @CrossOrigin(origins = "*") 또는 corsConfig 만들기
      final response = await dio.post("http://192.168.40.133:8080/api/goods" , data : obj );
      final data = response.data;
      print( data);
      // (1) 저장 성공된 자료를 추가하여 재렌더링  또는 (2) goodAll 재호출
      goodsAll(); // 한번더 서버 터치 vs setState(() { goodsList.add( data ); });
    }catch(e){ print(e); }
  }
  // [ 4-3 ] 전체조회 goodsAll 함수 정의 , useEffect( ()=>{} , [] ) 처럼 위젯이 최초로 열렸을때 1번 실행
  void goodsAll() async{
    try{
      final dio = Dio();
      final response = await dio.get("http://192.168.40.133:8080/api/goods/list");
      final data = response.data;
      print( data ); // 통신결과
      setState(() { goodsList = data; });// 통신결과를 함수 밖 변수(상태)에 업데이트(렌더링)
    }catch(e){ print(e); }
  }

  // [4-4] initState() 함수 오버라이딩 하여 위젯의 초기 상태
  @override  // 오버라이딩
  void initState() { goodsAll(); } // 현재 위젯이 최초 실행될때 1번 goodeAll 함수 실행
        ...
        child: Column(
          children: [
            ...
            Expanded(child: ListView.builder(
                ...
                itemBuilder: ( context, index ){  // 반복문역할
                  dynamic goods = goodsList[index]; // index번째 goods 1개 반환
                  // print(goods);
                  return Card( child: ListTile(
                    title: Text( goods['gname'] ), //  객채묭['속성명']
